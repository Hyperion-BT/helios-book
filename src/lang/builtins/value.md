...
Strict greater-equals comparison. If any rhs token has a smaller amount than the equivalent lhs token then `>=` returns `false`.
...
Strict greater-than comparison. If any rhs token has a less-or-equals amount than the equivalent lhs token then `>` returns `false`.
...
Strict less-equals comparison. If any rhs token has a greater amount than the equivalent lhs token, or doesn't exist in lhs, then `<=` returns `false`.
...
Strict less-than comparison. If any rhs token has a greater-or-equals amount than the equivalent lhs token, or doesn't exist in lhs, then `<` returns `false`.
